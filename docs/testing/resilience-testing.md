...
**Status (2026-08-14).** RES-01/02/03/07 are implemented and green in the backend suites. RES-04..06 are covered *as behaviour* by the tier-router matrix; the flag-toggling procedure against a running deployment is not done. **RES-09..12 are NOT passed** — the mobile app now exists and the mechanisms behind them are unit-tested (NetInfo-driven online manager, offline write guard, foreground refetch, registry prefetch, retry-same-bytes upload, the offline auth bootstrap), but every one of these four is a device scenario and **no physical device or emulator has run the app**. Per-scenario detail: `docs/testing/test-matrix.md`; the scripted device checklist is `docs/mobile/testing.md`.
