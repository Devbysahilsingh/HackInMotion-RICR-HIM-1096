...
**Timing.** Each hop gets `min(10s, time left)` against a **15s E2E deadline**. 10s + 10s exceeds 15s, so the per-hop ceiling alone cannot honour the budget; a hop with **no budget left is skipped, not started** (recorded as `deadline_exhausted`), which is what guarantees the local rules tier still runs inside the budget. Exhausting the budget degrades the answer's *tier*, never the response.
