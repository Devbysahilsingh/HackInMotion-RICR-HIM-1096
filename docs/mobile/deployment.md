...
- **Config:** `app.config.ts` (TypeScript, not `app.json` — the API base URL has to be read from the environment at build time and defaulted, which static JSON cannot do); name `Khetri` (OD-4), slug `him-1096-khetri`, scheme `khetri`, icon/adaptive icon in `mobile/assets/`, Android package id `in.him1096.khetri`, `versionCode` 1, `userInterfaceStyle: 'light'` (single light theme on purpose — the stated field context is bright sunlight). Permissions minimal: `CAMERA` + `ACCESS_FINE_LOCATION` declared, `RECORD_AUDIO` in `blockedPermissions`.
