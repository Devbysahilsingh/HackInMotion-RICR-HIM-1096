...
CORS: exact Vercel prod URL + localhost dev (env CORS_ORIGINS). No wildcard, credentials on auth path only. The prod entry is the **stable alias** (`https://hack-in-motion-ricr-him-1096.vercel.app`), never a per-deployment `<project>-<hash>.vercel.app` host — those are frozen per build, so pinning one blocks the live site: `cors` omits `Access-Control-Allow-Origin` entirely on a miss, which surfaces in the browser as a CORS failure while curl still sees a normal 200/401. Value lives in `render.yaml` (an allowlist is public, not a secret); changing it needs a backend redeploy.
