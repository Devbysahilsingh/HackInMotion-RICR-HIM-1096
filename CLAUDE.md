...
**Completed:** P0-3 development foundation · P0-4 dataset acquisition · P0-5 dataset audit · P0-5b rice healthy acquisition · P0-6/P0-6b dataset preparation + pre-training gates (no training) · **P1-1..P1-8 backend foundation** (Express hardening, 14 models + index assertions, full auth lifecycle with refresh-family reuse detection, ownership layer, Farms + Crops CRUD, sourced crop registry with versioned seed, pure stage engine). Decisions: ADR-021, ADR-022 (approved). **PHASE 1 COMPLETE (P1-1..P1-8, approved 2026-08-13)** — 235 backend tests green; ST-50/ST-01..05/ST-10/ST-70 green. **Next:** proposed for approval — see implementation-log.md. **Outstanding external item:** Render account + staging deploy (owner A).

**How to run what exists:** root `npm install` (installs tooling and enables the pre-commit hook) · `cd backend && npm install && npm run dev` (needs `backend/.env`; copy names from `.env.example`) · `cd web/frontend && npm install && npm run dev`. Repo-wide checks: `npm run lint`, `npm run format:check`. Backend: `npm test` (235 tests) · `npm run indexes:build` · `npm run seed:registry [--dry-run]` · `npm run smoke -- <url>`.
